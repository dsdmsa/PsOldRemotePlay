...
import com.my.psremoteplay.core.streaming.JvmVideoRenderer
...
import com.my.psremoteplay.feature.ps2.strategy.client.*
import java.io.DataInputStream
import java.net.Socket

class DesktopPs2ClientDependencies(
    ...
    override val streaming: StreamingDependencies = object : StreamingDependencies {
        ...
        override val videoRenderer: VideoRenderer = JvmVideoRenderer(_logger)
    ...
    /** ← Change this to swap the client video pipeline */
    ...
    private fun createClientStrategy(preset: StreamingPreset): VideoStreamClient = when (preset) {
        StreamingPreset.JPEG_TCP -> TcpJpegClient(_logger)
        StreamingPreset.JPEG_UDP -> UdpJpegClient(_logger)
    ...
    override fun connectControl(ip: String, port: Int, onServerInfo: (String) -> Unit): Boolean {
        return try {
            ...
            val helloPayload = """{"name":"Desktop Client"}""".toByteArray()
